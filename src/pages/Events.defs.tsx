...
const _DeleteEventButton = ({ event }: { event: Event }) => {
  const { mutateAsync: deleteCompany } = useDeleteEvent()

  const onDeleteCompany = async (event: Event): Promise<void> => {
    try {
      await deleteCompany(event.id)
      toast.success(<p>O fornecedor "{event.name}" foi excluído com sucesso!</p>)
    } catch (error: unknown) {
  ...
  return (
    <ConfirmationModal
      ...
      onAction={() => void onDeleteCompany(event)}
    >
      <Button
        data-tooltip-id={`delete-company-${event.id}`}
      ...
      <Tooltip id={`delete-company-${event.id}`} place="top" />
    </ConfirmationModal>
  )
}
